			    ...
			    enum nl80211_channel_type ch_type)
{
	...
	unsigned long flag = 0;/* this initialization is unnecessary, just
				* to avoid compile warning in some kernels */
	u8 reg_prsr_rsc;
	u8 reg_bw_opmode;
...
static void _rtl92d_phy_iq_calibrate(struct ieee80211_hw *hw,
									long result[][8], u8 t, bool is2t)
{
	...
	/* MAC settings */
	_rtl92d_phy_mac_setting_calibration(hw,
										iqk_mac_reg, rtlphy->iqk_mac_backup);
	/* Page B init */
	...
	for (i = 0; i < retrycount; i++) {
		...
		if (patha_ok == 0x03) {
			...
			RTPRINT(rtlpriv,
					FINIT, INIT_IQK, "Path A IQK Only  Tx Success!!\n");
...
void rtl92d_phy_reload_iqk_setting(struct ieee80211_hw *hw, u8 channel)
{
	...
	if (0 && !rtlphy->iqk_matrix_regsetting[indexforchannel].iqk_done &&
		...
		if (((!rtlhal->load_imrandiqk_setting_for2g)
			...
			if ((rtlphy->iqk_matrix_regsetting[indexforchannel].value[0][0] != 0)
				/*&&(regea4 != 0) */)
			 ...
			 * rtlhal->load_imrandiqk_setting_for2g = true; */
		}
	}
...
static void _rtl92d_phy_update_bbrf_for_easyconcurrent(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
	u32 bbregvalue = 0;
...
static void rtl92d_dualmac_easyconcurrent_bbrfreconfig(
...
static void _rtl92d_dualmac_reconfig_mgntinfo_forselfmac(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &(rtlpriv->phy);
...
static void _rtl92d_dualmac_reconfig_mgntinfo_foranothermac(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &(rtlpriv->phy);
	struct rtl_priv *buddy_priv = rtlpriv->buddy_priv;
	...
	if (buddy_priv == NULL) {
		...
		if ((buddy_priv->mac80211.opmode != NL80211_IFTYPE_AP &&
			...
			if (rtlhal->macphymode == DUALMAC_SINGLEPHY) {
				...
				if (sta != NULL && sta->ht_cap.ht_supported) {
					if (buddy_priv->mac80211.link_state >= MAC80211_LINKED) {
						RT_TRACE(rtlpriv, COMP_EASY_CONCURRENT, DBG_EMERG,
