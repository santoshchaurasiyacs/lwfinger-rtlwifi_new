				  ...
				  bool dl_whole_packets)
{
	...
	if (bcnvalid_reg & BIT(0) && rtlhal->b_enter_pnp_sleep) {
...
void rtl8821ae_set_hw_reg(struct ieee80211_hw *hw, u8 variable, u8 *val)
{
	...
	case HW_VAR_NAV_UPPER: {
			...
			rtl_write_byte(rtlpriv, REG_NAV_UPPER,
				((u8)((us_nav_upper + HAL_92C_NAV_UPPER_UNIT - 1)
...
bool _rtl8821ae_check_pcie_dma_hang(struct ieee80211_hw *hw)
{
	...
	if ((tmp & BIT(0)) || (tmp & BIT(1))) {
		RT_TRACE(COMP_INIT, DBG_LOUD, ("CheckPcieDMAHang8821AE(): "
						"true! Reset PCIE DMA!\n"));
		return true;
	} else {
		return false;
					 ...
					 bool in_watchdog)
{
	...
	/* We don't check for hang in watchdog in Linux,
	 * so mask this
	if (in_watchdog) {
...
void _rtl8821ae_get_wakeup_reason(struct ieee80211_hw *hw)
{
	...
	RT_TRACE(COMP_POWER, DBG_LOUD, ("WOL Read 0x1c7 = %02X\n", fw_reason));
...
void _rtl8821ae_enable_l1off(struct ieee80211_hw *hw)
{
	...
	if (!(tmp & (BIT(2) | BIT(3)))) {
		RT_TRACE(COMP_POWER | COMP_INIT, DBG_LOUD,
			 ("0x160(%#x)return!!\n", tmp));
		return;
	}
...
bool _rtl8821ae_wowlan_initialize_adapter(struct ieee80211_hw *hw)
{
	...
	rtl_write_byte(rtlpriv, REG_PCIE_CTRL_REG + 1, 0xFE);
	RT_TRACE(COMP_POWER, DBG_LOUD, ("Enable PCIE Rx DMA.\n"));
	...
	RT_TRACE(COMP_POWER, DBG_LOUD, ("Read REG_FTISR 0x13f = %#X\n", tmp));
...
static enum version_8821ae _rtl8821ae_read_chip_version(struct ieee80211_hw *hw)
{
	...
	case VERSION_NORMAL_TSMC_CHIP_1T1R_8812_C_CUT:
		RT_TRACE(COMP_INIT, DBG_LOUD,
			("Chip Version ID: "
			"VERSION_NORMAL_TSMC_CHIP_1T1R_8812 C CUT.\n"));
		break;
	case VERSION_NORMAL_TSMC_CHIP_2T2R_8812_C_CUT:
		RT_TRACE(COMP_INIT, DBG_LOUD,
			("Chip Version ID: "
			"VERSION_NORMAL_TSMC_CHIP_2T2R_8812 C CUT.\n"));
		break;
	...
	case VERSION_NORMAL_TSMC_CHIP_8821:
		RT_TRACE(COMP_INIT, DBG_LOUD,
			("Chip Version ID: "
			"VERSION_NORMAL_TSMC_CHIP_8821 A CUT.\n"));
		break;
	case VERSION_NORMAL_TSMC_CHIP_8821_B_CUT:
		RT_TRACE(COMP_INIT, DBG_LOUD,
			("Chip Version ID: "
			"VERSION_NORMAL_TSMC_CHIP_8821 B CUT.\n"));
		break;
	default:
		RT_TRACE(COMP_INIT, DBG_LOUD,
				     ...
				     enum nl80211_iftype type)
{
	...
	case NL80211_IFTYPE_AP:
		...
		break;
	default:
		RT_TRACE(COMP_ERR, DBG_EMERG, ("Network type "
						"%d not support!\n", type));
		return 1;
		break;
...
void rtl8821ae_card_disable(struct ieee80211_hw *hw)
{
	...
	if (!(b_support_remote_wakeup && mac->opmode == NL80211_IFTYPE_STATION)
	    || !rtlhal->b_enter_pnp_sleep) {
		RT_TRACE(COMP_INIT, DBG_DMESG, ("Normal Power off\n"));
		mac->link_state = MAC80211_NOLINK;
		opmode = NL80211_IFTYPE_UNSPECIFIED;
		_rtl8821ae_set_media_status(hw, opmode);
		_rtl8821ae_poweroff_adapter(hw);
	} else {
		RT_TRACE(COMP_INIT, DBG_DMESG, ("Wowlan Supported.\n"));
...
#endif
	/*	tmp = rtl_read_byte(rtlpriv, 0x3);
		printk("reg3 = 0x%x\n",tmp);
		rtl_write_byte(rtlpriv, 0x3, tmp & (~BIT(2)));
		tmp = rtl_read_byte(rtlpriv, 0x3);
		printk("reg3 = 0x%x\n",tmp); */
	...
	/* For wowlan+LPS+32k. */
	if (b_support_remote_wakeup && rtlhal->b_enter_pnp_sleep) {
	...
	u8 *hwinfo)
{
	...
	RT_TRACE(COMP_INIT, DBG_LOUD, ("hal_ReadPowerValueFromPROM8821ae(): "
					"hwinfo[0x%x]=0x%x\n",
					(eeAddr+1),
					hwinfo[eeAddr+1]));
	...
	if (autoload_fail) {
		RT_TRACE(COMP_INIT, DBG_LOUD, ("auto load fail : "
						"Use Default value!\n"));
...
static void _rtl8812ae_read_adapter_info(struct ieee80211_hw *hw, bool b_pseudo_test)
{
	...
	*/
	...
	/*board type*/
	rtlefuse->board_type =
		(((*(u8 *) &hwinfo[EEPROM_RF_BOARD_OPTION]) & 0xE0) >> 5);
	...
	if (rtlefuse->autoload_failflag == false) {
		rtlefuse->antenna_div_cfg =
			*(u8 *) &hwinfo[EEPROM_RF_BOARD_OPTION] & 0x18 >> 3;
		if (*(u8 *) &hwinfo[EEPROM_RF_BOARD_OPTION] == 0xff)
	...
	if (!autoload_fail) {
		if (hwinfo[EEPROM_RFE_OPTION] & BIT(7)) {
			...
			if (rtlhal->rfe_type == 4 &&
			    (rtlhal->external_pa_5g == true ||
			     rtlhal->external_pa_2g == true ||
			     rtlhal->external_lna_5g == true ||
			     rtlhal->external_lna_2g == true)) {
				if (rtlhal->hw_type == HARDWARE_TYPE_RTL8812AE)
					rtlhal->rfe_type = 2;
			}
		}
...
static void _rtl8821ae_read_adapter_info(struct ieee80211_hw *hw, bool b_pseudo_test)
{
	...
	if (rtlefuse->autoload_failflag == false) {
		rtlefuse->antenna_div_cfg = (hwinfo[EEPROM_RF_BOARD_OPTION] & 0x18) >> 3;
		if (hwinfo[EEPROM_RF_BOARD_OPTION] == 0xff)
			rtlefuse->antenna_div_cfg = 0;
...
#if 0
	/* not support stbc & ldpc now */
	mac->vht_stbc_cap = STBC_VHT_ENABLE_RX;
	if (rtlphy->rf_type == RF_2T2R)
		mac->vht_stbc_cap |= STBC_VHT_ENABLE_TX;
	mac->vht_stbc_cap = 0;
	mac->vht_cur_stbc = mac->vht_stbc_cap;
	mac->ht_cur_stbc = mac->ht_stbc_cap = mac->vht_stbc_cap;

	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8821AE ||
	    IS_VENDOR_8812A_C_CUT(rtlhal->version))
		mac->vht_ldpc_cap = LDPC_VHT_ENABLE_TX | LDPC_VHT_ENABLE_RX;
	mac->vht_stbc_cap = 0;
	mac->vht_cur_ldpc = mac->vht_ldpc_cap;
#endif
...
/*	u8 b_curtxbw_40mhz = (sta->ht_cap.cap & IEEE80211_HT_CAP_SUP_WIDTH_20_40)
				? 1 : 0;
	u8 b_curtxbw_80mhz = (sta->vht_cap.vht_supported &&
		(!(sta->vht_cap.cap &
		...
		struct ieee80211_sta *sta, u8 rssi_level)
{
	...
	RT_TRACE(COMP_RATR, DBG_LOUD, ("wireless mode = 0x%x\n", wirelessmode));
		...
		struct ieee80211_sta *sta, u8 rssi_level)
{
	...
	if (rtlpriv->dm.b_useramask)
		...
		/*RT_TRACE(COMP_RATR,DBG_LOUD,("rtl8821ae_update_hal_rate_tbl()"
