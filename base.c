		...
		struct wireless_dev *wdev, const void *data, int len)
{
	...
	rtlpriv->btcoexist.btc_info.ap_num = rtl_data_to_int(rtlpriv, data, len);
...
static void _rtl_init_mac80211(struct ieee80211_hw *hw)
{
	...
	hw->wiphy->n_vendor_commands = sizeof(rtl_vendor_cmds)/sizeof(rtl_vendor_cmds[0]);
#endif
...
int rtl_init_core(struct ieee80211_hw *hw)
{
	...
	spin_lock_init(&rtlpriv->locks.socket_lock);
	/* <5> init list */
...
#else
		if (NL80211_BAND_2GHZ == hw->conf.channel->band) {
#endif
			switch (desc_rate) {
			case DESC_RATE1M:
				rate_idx = 0;
...
void rtl_scan_list_expire(struct ieee80211_hw *hw)
{
	...
	list_for_each_entry_safe(entry, next, &rtlpriv->scan_list.list, list) {
		...
		if (jiffies_to_msecs(jiffies - entry->age) < 180000)	/* 180 seconds */
			continue;
