							     ...
							     *hw)
{
	...
	long val_y, ele_c = 0;
	u8 ofdm_index[2], cck_index, cck_index_old = 0;
...
*/
	...
	if (bt_state != rtlpcipriv->btcoexist.bt_cur_state) {
		...
		if (rtlpcipriv->btcoexist.reg_bt_sco == 3) {
			...
			bt_state = bt_state |
				   ...
				   == 1) ? 0 : BIT_OFFSET_LEN_MASK_32(1, 1)) |
	...
	if (bt_state && rtlpcipriv->btcoexist.reg_bt_sco == 3) {
		...
		if (cur_service_type != rtlpcipriv->btcoexist.bt_service) {
			rtlpcipriv->btcoexist.bt_service = cur_service_type;
			bt_state = bt_state |
...
static u8 rtl92c_bt_rssi_state_change(struct ieee80211_hw *hw)
{
	...
	if (curr_bt_rssi_state != rtlpcipriv->btcoexist.bt_rssi_state) {
		rtlpcipriv->btcoexist.bt_rssi_state = curr_bt_rssi_state;
		return true;
	} else {
		return false;
	}
...
void rtl92c_dm_bt_coexist(struct ieee80211_hw *hw)
{
	...
	if ((rtlpcipriv->btcoexist.bt_coexistence) &&
		...
		if (b_wifi_connect_change || bt_state_change || b_rssi_state_change) {
			...
			if (rtlpcipriv->btcoexist.bt_cur_state) {
				if (rtlpcipriv->btcoexist.bt_ant_isolation) {
					...
					if (rtlpriv->mac80211.vendor == PEER_CISCO &&
						...
						if ((rtlpcipriv->btcoexist.bt_service == BT_BUSY) &&
						    (rtlpcipriv->btcoexist.bt_rssi_state &
						     BT_RSSI_STATE_NORMAL_POWER)) {
							rtl_write_byte(rtlpriv, REG_GPIO_MUXCFG, 0xa0);
						} else if ((rtlpcipriv->btcoexist.bt_service ==
								BT_OTHER_ACTION) && (rtlpriv->mac80211.mode <
								WIRELESS_MODE_N_24G) &&
								(rtlpcipriv->btcoexist.bt_rssi_state &
								BT_RSSI_STATE_SPECIAL_LOW)) {
							rtl_write_byte(rtlpriv, REG_GPIO_MUXCFG, 0xa0);
						} else if (rtlpcipriv->btcoexist.bt_service == BT_PAN) {
					...
					if (rtlpcipriv->btcoexist.bt_service == BT_PAN) {
					...
					if (rtlpcipriv->btcoexist.bt_rssi_state &
						BT_RSSI_STATE_NORMAL_POWER) {
						if (rtlpcipriv->btcoexist.bt_service == BT_OTHERBUSY) {
							rtlpcipriv->btcoexist.bt_edca_ul = 0x5ea72b;
							rtlpcipriv->btcoexist.bt_edca_dl = 0x5ea72b;
						} else if (rtlpcipriv->btcoexist.bt_service == BT_BUSY) {
							rtlpcipriv->btcoexist.bt_edca_ul = 0x5eb82f;
							rtlpcipriv->btcoexist.bt_edca_dl = 0x5eb82f;
						} else if (rtlpcipriv->btcoexist.bt_service == BT_SCO) {
							if (rtlpcipriv->btcoexist.ratio_tx > 160) {
								rtlpcipriv->btcoexist.bt_edca_ul = 0x5ea72f;
								rtlpcipriv->btcoexist.bt_edca_dl = 0x5ea72f;
							} else {
								rtlpcipriv->btcoexist.bt_edca_ul = 0x5ea32b;
								rtlpcipriv->btcoexist.bt_edca_dl = 0x5ea42b;
							}
						} else {
						...
						if ((rtlpcipriv->btcoexist.bt_service != BT_IDLE) &&
							...
							rtlpcipriv->btcoexist.bt_edca_ul = 0x5eb82b;
							rtlpcipriv->btcoexist.bt_edca_dl = 0x5eb82b;
						}
					} else {
						rtlpcipriv->btcoexist.bt_edca_ul = 0;
						rtlpcipriv->btcoexist.bt_edca_dl = 0;
