...
============================================================
*/
static u8 halbtc8821a1ant_bt_rssi_state(
	u8			level_num,
	u8			rssi_thresh,
	u8			rssi_thresh1) {
	long			bt_rssi = 0;
...
static u8 halbtc8821a1ant_WifiRssiState(
...
static void halbtc8821a1ant_update_ra_mask(
	struct btc_coexist *btcoexist,
	bool force_exec,
	u32 dis_rate_mask) {
	...
	if (force_exec || (coex_dm->pre_arfr_type != coex_dm->cur_arfr_type)) {
		...
		case 0:	/* normal mode*/
				btcoexist->btc_write_4byte(btcoexist,
					 0x430, coex_dm->backup_arfr_cnt1);
				btcoexist->btc_write_4byte(btcoexist,
					 0x434, coex_dm->backup_arfr_cnt2);
				break;
		case 1:
				btcoexist->btc_get(btcoexist,
					 BTC_GET_BL_WIFI_UNDER_B_MODE, &wifi_under_b_mode);
...
static void halbtc8821a1ant_retry_limit(
	struct btc_coexist *btcoexist,
	bool force_exec,
	u8 type) {
	...
	if (force_exec || (coex_dm->pre_retry_limit_type !=
		...
		case 0:	/* normal mode*/
				btcoexist->btc_write_2byte(btcoexist,
					 0x42a, coex_dm->backup_retry_limit);
				break;
...
static void halbtc8821a1ant_ampdu_max_time(
	struct btc_coexist *btcoexist,
	bool force_exec,
	u8 type) {
	...
	if (force_exec || (coex_dm->pre_ampdu_time_type !=
		...
		case 0:	/* normal mode*/
				btcoexist->btc_write_1byte(btcoexist,
					 0x456, coex_dm->backup_ampdu_max_time);
				break;
...
static void halbtc8821a1ant_limited_tx(
	struct btc_coexist *btcoexist,
	bool force_exec,
	u8 ra_mask_type,
	u8 arfr_type,
...
static void halbtc8821a1ant_limited_rx(
	struct btc_coexist *btcoexist,
	bool force_exec,
	bool rej_ap_agg_pkt,
	...
	btcoexist->btc_set(btcoexist,
		 BTC_SET_BL_BT_CTRL_AGG_SIZE, &bt_ctrl_rx_agg_size);
	/* aggregation buf size, only work when BT control Rx aggregation size.*/
	btcoexist->btc_set(btcoexist,
		 BTC_SET_U1_AGG_BUF_SIZE, &rx_agg_size);
	/* real update aggregation setting*/
	btcoexist->btc_set(btcoexist,
		 BTC_SET_ACT_AGGREGATE_CTRL, NULL);


}

static void halbtc8821a1ant_monitor_bt_ctr(
	struct btc_coexist *btcoexist) {
...
static void halbtc8821a1ant_query_bt_info(
	struct btc_coexist *btcoexist) {
...
static void halbtc8821a1ant_update_bt_link_info(
	struct btc_coexist *btcoexist) {
...
static u8 halbtc8821a1ant_action_algorithm(
	struct btc_coexist *btcoexist) {
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
	bool				bt_hs_on = false;
...
static void halbtc8821a1ant_bt_auto_report(
	struct btc_coexist *btcoexist,
	bool force_exec,
	bool enable_auto_report) {
...
static void halbtc8821a1ant_set_sw_penalty_tx_rate_adaptive(
	struct btc_coexist *btcoexist,
	bool low_penalty_ra) {
...
static void halbtc8821a1ant_set_coex_table(
	struct btc_coexist *btcoexist,
	u32 val0x6c0,
	u32 val0x6c4,
	u32 val0x6c8,
...
static void halbtc8821a1ant_coex_table(
	struct btc_coexist *btcoexist,
	bool force_exec,
	u32 val0x6c0,
	u32 val0x6c4,
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_SW,
		"[BTCoex], %s write Coex Table 0x6c0 = 0x%x, 0x6c4 = 0x%x, "
		"0x6c8 = 0x%x, 0x6cc = 0x%x\n",
	...
	halbtc8821a1ant_set_coex_table(btcoexist,
...
static void halbtc8821a1ant_coex_table_with_type(
	struct btc_coexist *btcoexist,
	bool force_exec,
	u8 type) {
	switch (type) {
	case 0:
			halbtc8821a1ant_coex_table(btcoexist,
	...
	case 2:
			halbtc8821a1ant_coex_table(btcoexist,
				 force_exec, 0x5a5a5a5a, 0x5a5a5a5a, 0xffffff, 0x3);
			break;
	case 3:
			halbtc8821a1ant_coex_table(btcoexist,
				 force_exec, 0x55555555, 0xaaaaaaaa, 0xffffff, 0x3);
			break;
	case 4:
			halbtc8821a1ant_coex_table(btcoexist,
	...
	case 7:
			halbtc8821a1ant_coex_table(btcoexist,
				 force_exec, 0x5afa5afa, 0x5afa5afa, 0xffffff, 0x3);
			break;
	default:
			break;
	}
...
static void halbtc8821a1ant_set_fw_ignore_wlan_act(
	struct btc_coexist *btcoexist,
	bool enable) {
	u8	h2c_parameter[1] = {0};
...
static void halbtc8821a1ant_ignore_wlan_act(
	struct btc_coexist *btcoexist,
	bool force_exec,
	bool enable) {
...
static void halbtc8821a1ant_set_fw_pstdma(
	struct btc_coexist *btcoexist,
	u8 byte1,
	u8 byte2,
	u8 byte3,
...
static void halbtc8821a1ant_set_lps_rpwm(
	struct btc_coexist *btcoexist,
	u8 lps_val,
	u8 rpwm_val) {
	u8	lps = lps_val;
...
static void halbtc8821a1ant_lps_rpwm(
	struct btc_coexist *btcoexist,
	bool force_exec,
	u8 lps_val,
	u8 rpwm_val) {
...
static void halbtc8821a1ant_sw_mechanism(
	struct btc_coexist *btcoexist,
	bool low_penalty_ra) {
...
static void halbtc8821a1ant_set_ant_path(
	struct btc_coexist *btcoexist,
	u8 ant_pos_type,
	bool init_hw_cfg,
	bool wifi_off) {
...
static void halbtc8821a1ant_ps_tdma(
	struct btc_coexist *btcoexist,
	bool force_exec,
	bool turn_on,
	u8 type) {
	...
	if (turn_on) {
		switch (type) {
		default:
				halbtc8821a1ant_set_fw_pstdma(btcoexist,
...
static bool halbtc8821a1ant_is_common_action(
	struct btc_coexist *btcoexist) {
...
static void halbtc8821a1ant_tdma_duration_adjust_for_acl(
	struct btc_coexist *btcoexist,
	u8 wifi_status) {
	...
	"[BTCoex], retry_count = %d\n", retry_count));*/
		...
		if (retry_count == 0) {
			...
			if (dn == 2) {
				/* if retry count< 3 for 2*2 seconds, shrink wifi duration */
				if (wait_count <= 2)
					m++; /* avoid bounce in two levels */
				else
					m = 1;

				if (m >= 20)/* m max value is 20, max time is 120 second,
...
static void halbtc8821a1ant_ps_tdma_check_for_power_save_state(
	struct btc_coexist *btcoexist,
	bool new_ps_state) {
	u8	lps_mode = 0x0;
...
static void halbtc8821a1ant_power_save_state(
	struct btc_coexist *btcoexist,
	u8 ps_type,
	u8 lps_val,
	u8 rpwm_val) {
...
static void halbtc8821a1ant_coex_under_5g(
	struct btc_coexist *btcoexist) {
...
static void halbtc8821a1ant_action_wifi_only(
	struct btc_coexist *btcoexist) {
	halbtc8821a1ant_coex_table_with_type(btcoexist, NORMAL_EXEC, 0);
	halbtc8821a1ant_ps_tdma(btcoexist, NORMAL_EXEC, false, 9);
}

static void halbtc8821a1ant_monitor_bt_enable_disable(
	struct btc_coexist *btcoexist) {
...
static void halbtc8821a1ant_action_sco(
	struct btc_coexist *btcoexist) {
	halbtc8821a1ant_sw_mechanism(btcoexist, true);
}

static void halbtc8821a1ant_action_hid(
	struct btc_coexist *btcoexist) {
	halbtc8821a1ant_sw_mechanism(btcoexist, true);
}

/*A2DP only / PAN(EDR) only/ A2DP+PAN(HS)*/
static void halbtc8821a1ant_action_a2dp(
	struct btc_coexist *btcoexist) {
	halbtc8821a1ant_sw_mechanism(btcoexist, false);
}

static void halbtc8821a1ant_action_a2dp_pan_hs(
	struct btc_coexist *btcoexist) {
	halbtc8821a1ant_sw_mechanism(btcoexist, false);
}
...
/*PAN(HS) only*/
static void halbtc8821a1ant_action_pan_hs(
	struct btc_coexist *btcoexist) {
	halbtc8821a1ant_sw_mechanism(btcoexist, false);
}

/*PAN(EDR)+A2DP*/
static void halbtc8821a1ant_action_pan_edr_a2dp(
	struct btc_coexist *btcoexist) {
	halbtc8821a1ant_sw_mechanism(btcoexist, false);
}

static void halbtc8821a1ant_action_pan_edr_hid(
	struct btc_coexist *btcoexist) {
	halbtc8821a1ant_sw_mechanism(btcoexist, true);
}
...
static void halbtc8821a1ant_action_bt_inquiry(
	struct btc_coexist *btcoexist) {
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
...
static void halbtc8821a1ant_action_bt_sco_hid_only_busy(
	struct btc_coexist *btcoexist,
	u8 wifi_status) {
	/* tdma and coex table*/
	halbtc8821a1ant_ps_tdma(btcoexist, NORMAL_EXEC, true, 5);
...
static void halbtc8821a1ant_action_wifi_connected_bt_acl_busy(
	struct btc_coexist *btcoexist,
	u8 wifi_status)
{
	u8		bt_rssi_state;
	...
	if (bt_link_info->hid_only) {
		...
		if ((bt_rssi_state == BTC_RSSI_STATE_HIGH) ||
			(bt_rssi_state == BTC_RSSI_STATE_STAY_HIGH)) {
			 halbtc8821a1ant_tdma_duration_adjust_for_acl(btcoexist,
				 wifi_status);
		} else {
			/*for low BT RSSI*/
			halbtc8821a1ant_ps_tdma(btcoexist, NORMAL_EXEC, true, 11);
...
static void halbtc8821a1ant_action_wifi_not_connected_asso_auth_scan(
	struct btc_coexist *btcoexist)
{
	halbtc8821a1ant_power_save_state(btcoexist,
		 BTC_PS_WIFI_NATIVE, 0x0, 0x0);
...
static void halbtc8821a1ant_action_wifi_connected_special_packet(
	struct btc_coexist *btcoexist) {
...
static void halbtc8821a1ant_action_wifi_connected(
	struct btc_coexist *btcoexist) {
...
static void halbtc8821a1ant_run_sw_coexist_mechanism(
	struct btc_coexist *btcoexist) {
	u8	algorithm = 0;
...
static void halbtc8821a1ant_run_coexist_mechanism(
	struct btc_coexist *btcoexist) {
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
	...
	btcoexist->btc_set(btcoexist,
		 BTC_SET_BL_INC_SCAN_DEV_NUM, &increase_scan_dev_num);
	...
	if (!bt_link_info->sco_exist && !bt_link_info->hid_exist) {
		...
		if (wifi_connected) {
			wifi_rssi_state = halbtc8821a1ant_WifiRssiState(btcoexist,
				 1, 2, 30, 0);
	...
	halbtc8821a1ant_limited_rx(btcoexist,
		 NORMAL_EXEC, false, bt_ctrl_agg_buf_size, agg_buf_size);
...
static void halbtc8821a1ant_init_coex_dm(
	struct btc_coexist *btcoexist) {
...
static void halbtc8821a1ant_init_hw_config(
	struct btc_coexist *btcoexist,
	bool back_up) {
	u8	u1_tmp = 0;
	bool			wifi_under_5g = false;
	...
	if (back_up) {
		...
		coex_dm->backup_arfr_cnt2 = btcoexist->btc_read_4byte(btcoexist,
			 0x434);
		coex_dm->backup_retry_limit = btcoexist->btc_read_2byte(btcoexist,
			 0x42a);
		coex_dm->backup_ampdu_max_time = btcoexist->btc_read_1byte(btcoexist,
			 0x456);
	}
	...
	if (wifi_under_5g)
		halbtc8821a1ant_set_ant_path(btcoexist,
			 BTC_ANT_PATH_BT, true, false);
	else
		halbtc8821a1ant_set_ant_path(btcoexist,
			 BTC_ANT_PATH_PTA, true, false);
	/* PTA parameter*/
...
ex_halbtc8821a1ant_display_coex_info(
	...
	btcoexist->btc_get(btcoexist,
		 BTC_GET_U1_WIFI_DOT11_CHNL, &wifi_dot11_chnl);
	btcoexist->btc_get(btcoexist,
