 ...
 *============================================================
 */
static u8 halbtc8821a2ant_bt_rssi_state(
	u8			level_num,
	u8			rssi_thresh,
	u8			rssi_thresh1
	) {
	...
	if (level_num == 2) {
		...
		if ((coex_sta->pre_bt_rssi_state == BTC_RSSI_STATE_LOW) ||
			...
			if (bt_rssi >=
				(rssi_thresh1+BTC_RSSI_COEX_THRESH_TOL_8821A_2ANT)) {
				bt_rssi_state = BTC_RSSI_STATE_HIGH;
				BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_BT_RSSI_STATE,
					"[BTCoex], BT Rssi state switch to High\n");
...
static u8 halbtc8821a2ant_wifi_rssi_state(
...
static void halbtc8821a2ant_monitor_bt_enable_disable(
	struct btc_coexist *btcoexist
	)
{
	...
	if (pre_bt_disabled != bt_disabled) {
		BTC_PRINT(BTC_MSG_ALGORITHM,
			ALGO_BT_MONITOR, "[BTCoex], BT is from %s to %s!!\n",
		...
		if (bt_hs_on) {
			if (bt_hs_rssi > 37) {
				BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW,
					"[BTCoex], Need to decrease "
					"bt power for HS mode!!\n");
				ret = true;
			}
		} else {
			...
			(bt_rssi_state == BTC_RSSI_STATE_STAY_HIGH)) {
				...
				BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW,
					"[BTCoex], Need to decrease bt power"
					" for Wifi is connected!!\n");
				ret = true;
			}
		}
	}
...
static void halbtc8821a2ant_set_fw_dac_swing_level(
	struct btc_coexist *btcoexist,
	u8 dac_swing_lvl
	)
{
	...
	)
{
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_EXEC, "[BTCoex], "
		"decrease Bt Power : %s, FW write 0x62 = 0x%x\n",
...
static void halbtc8821a2ant_set_fw_bt_lna_constrain(struct btc_coexist *btcoexist,
	bool bt_lna_cons_on
	) {
...
static void halbtc8821a2ant_set_bt_lna_constrain(struct btc_coexist *btcoexist,
	bool force_exec,
	bool bt_lna_cons_on
	)
{
	...
	if (!force_exec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL,
			"[BTCoex], pre_bt_lna_constrain = %d,"
			"cur_bt_lna_constrain = %d\n",
	...
	halbtc8821a2ant_set_fw_bt_lna_constrain(btcoexist,
		coex_dm->cur_bt_lna_constrain);
	...
	)
{
	...
	if (!force_exec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL,
			"[BTCoex], pre_bt_psd_mode = 0x%x, "
			"cur_bt_psd_mode = 0x%x\n",
...
static void halbtc8821a2ant_set_bt_auto_report(struct btc_coexist *btcoexist,
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_EXEC, "[BTCoex], "
		"BT FW auto report : %s, FW write 0x68 = 0x%x\n",
...
{
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW, "[BTCoex], %s "
		"BT Auto report = %s\n", (force_exec ? "force to":""),
		((enable_auto_report) ? "Enabled":"Disabled"));
	...
	if (!force_exec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL,
			"[BTCoex], pre_bt_auto_report = %d,"
			" cur_bt_auto_report = %d\n",
...
static void halbtc8821a2ant_fw_dac_swing_lvl(struct btc_coexist *btcoexist,
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW, "[BTCoex], %s "
		"set FW Dac Swing level = %d\n",
		(force_exec ? "force to":""), fw_dac_swing_lvl);
	coex_dm->cur_fw_dac_swing_lvl = fw_dac_swing_lvl;

	if (!force_exec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL,
			"[BTCoex], pre_fw_dac_swing_lvl = %d, "
			"cur_fw_dac_swing_lvl = %d\n",
...
static void halbtc8821a2ant_set_sw_rf_rx_lpf_corner(struct btc_coexist *btcoexist,
	...
	if (rx_rf_shrink_on) {
		...
		if (btcoexist->initilized) {
			...
			btcoexist->btc_set_rf_reg(btcoexist, BTC_RF_A,
				0x1e, 0xfffff, coex_dm->bt_rf0x1e_backup);
		}
	}
}
	...
	)
{
	...
	if (!force_exec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_SW_DETAIL,
			"[BTCoex], pre_rf_rx_lpf_shrink = %d, "
			"cur_rf_rx_lpf_shrink = %d\n",
...
static void halbtc8821a2ant_SetSwPenaltyTxRateAdaptive(struct btc_coexist *btcoexist,
	bool low_penalty_ra
	) {
	...
	)
{
	...
	if (!force_exec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_SW_DETAIL,
			"[BTCoex], pre_low_penalty_ra = %d, "
			"cur_low_penalty_ra = %d\n",
			coex_dm->pre_low_penalty_ra, coex_dm->cur_low_penalty_ra);

		if (coex_dm->pre_low_penalty_ra == coex_dm->cur_low_penalty_ra)
			return;
	}
	halbtc8821a2ant_SetSwPenaltyTxRateAdaptive(btcoexist,
		coex_dm->cur_low_penalty_ra);

	coex_dm->pre_low_penalty_ra = coex_dm->cur_low_penalty_ra;
}

static void halbtc8821a2ant_set_dac_swing_reg(struct btc_coexist *btcoexist, u32 level)
{
	u8 val = (u8)level;
...
static void halbtc8821a2ant_set_sw_fulltime_dac_swing(struct btc_coexist *btcoexist,
	...
	)
{
	...
	if (!force_exec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_SW_DETAIL,
			"[BTCoex], pre_dac_swing_on = %d, "
			"pre_dac_swing_lvl = 0x%x, "
	...
	mdelay(30);
	halbtc8821a2ant_set_sw_fulltime_dac_swing(btcoexist,
...
static void halbtc8821a2ant_set_coex_table(
	struct btc_coexist *btcoexist,
	u32 val0x6c0,
	u32 val0x6c4,
	u32 val0x6c8,
				       ...
				       u32 val0x6c4, u32 val0x6c8, u8 val0x6cc)
{
	...
	if (!force_exec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_SW_DETAIL,
			"[BTCoex], pre_val0x6c0 = 0x%x, "
			"pre_val0x6c4 = 0x%x, pre_val0x6c8 = 0x%x, "
			"pre_val0x6cc = 0x%x !!\n",
	...
	halbtc8821a2ant_set_coex_table(btcoexist, val0x6c0,
...
static void halbtc8821a2ant_set_fw_ignore_wlan_act(struct btc_coexist *btcoexist, bool enable)
{
	u8 h2c_parameter[1] = {0};
...
static void halbtc8821a2ant_ignore_wlan_act(struct btc_coexist *btcoexist, bool force_exec, bool enable)
{
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW,
...
static void halbtc8821a2ant_set_fw_pstdma(struct btc_coexist *btcoexist, u8 byte1, u8 byte2, u8 byte3, u8 byte4, u8 byte5)
{
...
static void halbtc8821a2ant_sw_mechanism1(struct btc_coexist *btcoexist, bool shrink_rx_lpf, bool low_penalty_ra, bool limited_dig, bool bt_lna_constrain)
{
	u32 wifi_bw;
	...
	/* no limited DIG
	 * halbtc8821a2ant_set_bt_lna_constrain(btcoexist,
		NORMAL_EXEC, bBTLNAConstrain);
	 */
}

static void halbtc8821a2ant_sw_mechanism2(struct btc_coexist *btcoexist, bool agc_table_shift, bool adc_back_off, bool sw_dac_swing, u32 dac_swing_lvl)
{
	...
	halbtc8821a2ant_dac_swing(btcoexist,
		NORMAL_EXEC, sw_dac_swing, sw_dac_swing);
}

static void halbtc8821a2ant_set_ant_path(struct btc_coexist *btcoexist, u8 ant_pos_type, bool init_hw_cfg, bool wifi_off)
{
	...
	if (init_hw_cfg) {
		...
		if (board_info->btdm_ant_pos == BTC_ANTENNA_AT_MAIN_PORT) {
			...
			h2c_parameter[0] = 1;
			h2c_parameter[1] = 1;
			btcoexist->btc_fill_h2c(btcoexist,
...
static void halbtc8821a2ant_ps_tdma(struct btc_coexist *btcoexist, bool force_exec, bool turn_on, u8 type)
{
	...
	if (turn_on) {
		...
		case 1:
		default:
				halbtc8821a2ant_set_fw_pstdma(btcoexist,
...
static bool halbtc8821a2ant_is_common_action(struct btc_coexist *btcoexist)
{
	...
	halbtc8821a2ant_coex_table(btcoexist,
		NORMAL_EXEC, 0x55ff55ff, 0x5afa5afa, 0xffff, 0x3);
	...
	if (!wifi_connected &&
		...
		halbtc8821a2ant_sw_mechanism1(btcoexist,
			false, false, false, false);
		halbtc8821a2ant_sw_mechanism2(btcoexist,
...
static void halbtc8821a2ant_tdma_duration_adjust(struct btc_coexist *btcoexist,
	IN	bool			sco_hid,
	IN	bool			tx_pause,
	IN	u8			max_interval
	) {
	...
	if (coex_dm->reset_tdma_adjust) {
		...
		/* <=3 retry in the last 2-second duration */
			...
			/* if retry count< 3 for 2*2 seconds,
				...
				BTC_PRINT(BTC_MSG_ALGORITHM,
					ALGO_TRACE_FW_DETAIL,
					"[BTCoex], Decrease wifi "
					"duration for retryCounter<3!!\n");
			}
		} else {
	...
	if (coex_dm->cur_ps_tdma != coex_dm->tdma_adj_type) {
		...
		if (!scan && !link && !roam) {
			halbtc8821a2ant_ps_tdma(btcoexist,
				NORMAL_EXEC, true, coex_dm->tdma_adj_type);
		} else {
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL,
	...
	/* when halbtc8821a2ant_tdma_duration_adjust() is called,
		fw dac swing is included in the function.
	 *if (coex_dm->tdma_adj_type == 71)
	 *	halbtc8821a2ant_fw_dac_swing_lvl(btcoexist, NORMAL_EXEC, 0xc);
		//Skip because A2DP get worse at HT40
	 * else
	 */
...
static void halbtc8821a2ant_action_sco(struct btc_coexist *btcoexist)
{
	...
	/* for SCO quality at 11b/g mode */
		halbtc8821a2ant_coex_table(btcoexist,
			NORMAL_EXEC, 0x5a5a5a5a, 0x5a5a5a5a, 0xffff, 0x3);
	} else {
		/* for SCO quality & wifi performance balance at 11n mode */
		halbtc8821a2ant_coex_table(btcoexist,
			NORMAL_EXEC, 0x5aea5aea, 0x5aea5aea, 0xffff, 0x3);
	}
	...
	if (BTC_WIFI_BW_HT40 == wifi_bw) {
		...
		if ((bt_rssi_state == BTC_RSSI_STATE_HIGH) ||
			(bt_rssi_state == BTC_RSSI_STATE_STAY_HIGH)) {
			halbtc8821a2ant_ps_tdma(btcoexist,
				NORMAL_EXEC, false, 0); /*for voice quality*/
		} else {
			halbtc8821a2ant_ps_tdma(btcoexist,
				NORMAL_EXEC, false, 0); /*for voice quality*/
		}
...
static void halbtc8821a2ant_action_hid(struct btc_coexist *btcoexist)
{
	...
	/* for HID at 11b/g mode */
		halbtc8821a2ant_coex_table(btcoexist,
			NORMAL_EXEC, 0x55ff55ff, 0x5a5a5a5a, 0xffff, 0x3);
	} else {
	/* for HID quality & wifi performance balance at 11n mode */
		halbtc8821a2ant_coex_table(btcoexist,
			NORMAL_EXEC, 0x55ff55ff, 0x5aea5aea, 0xffff, 0x3);
	}
...
static void halbtc8821a2ant_action_pan_edr(struct btc_coexist *btcoexist)
{
	...
	/* for HID at 11b/g mode */
		halbtc8821a2ant_coex_table(btcoexist,
			NORMAL_EXEC, 0x55ff55ff, 0x5aff5aff, 0xffff, 0x3);
	} else {
	/* for HID quality & wifi performance balance at 11n mode */
		halbtc8821a2ant_coex_table(btcoexist,
			NORMAL_EXEC, 0x55ff55ff, 0x5aff5aff, 0xffff, 0x3);
...
static void halbtc8821a2ant_action_pan_edr_a2dp(struct btc_coexist *btcoexist)
{
	...
	if (BTC_WIFI_BW_HT40 == wifi_bw) {
		...
		if ((bt_rssi_state == BTC_RSSI_STATE_HIGH) ||
			...
			if (bt_info_ext&BIT0) {
			/* a2dp basic rate */
				halbtc8821a2ant_tdma_duration_adjust(btcoexist,
					false, false, 3);
			} else {
			/* a2dp edr rate */
				halbtc8821a2ant_tdma_duration_adjust(btcoexist,
...
static void halbtc8821a2ant_action_pan_edr_hid(struct btc_coexist *btcoexist)
{
	...
	/* for HID at 11b/g mode */
		halbtc8821a2ant_coex_table(btcoexist,
			NORMAL_EXEC, 0x55ff55ff, 0x5a5f5a5f, 0xffff, 0x3);
	} else {
	/* for HID quality & wifi performance balance at 11n mode */
		halbtc8821a2ant_coex_table(btcoexist,
			NORMAL_EXEC, 0x55ff55ff, 0x5a5f5a5f, 0xffff, 0x3);
...
/* HID+A2DP+PAN(EDR) */
static void halbtc8821a2ant_action_hid_a2dp_pan_edr(struct btc_coexist *btcoexist)
{
	u8		wifi_rssi_state, bt_rssi_state, bt_info_ext;
	u32		wifi_bw;
...
static void halbtc8821a2ant_action_hid_a2dp( struct btc_coexist *btcoexist)
{
	...
	if (BTC_WIFI_BW_LEGACY == wifi_bw) {
	/* for HID at 11b/g mode */
/*Allen		halbtc8821a2ant_coex_table(btcoexist,
	NORMAL_EXEC, 0x55ff55ff, 0x5a5a5a5a, 0xffff, 0x3);*/
		halbtc8821a2ant_coex_table(btcoexist,
			NORMAL_EXEC, 0x55ff55ff, 0x5f5b5f5b, 0xffffff, 0x3);
	} else {
		/*for HID quality & wifi performance balance at 11n mode*/
/*Allen		halbtc8821a2ant_coex_table(btcoexist,
	NORMAL_EXEC, 0x55ff55ff, 0x5a5a5a5a, 0xffff, 0x3);*/
...
/*				halbtc8821a2ant_tdma_duration_adjust(btcoexist,
	true, false, 2); */
				...
				/* a2dp edr rate */
/*Allen				halbtc8821a2ant_tdma_duration_adjust(btcoexist,
	true, false, 2);*/
				halbtc8821a2ant_tdma_duration_adjust(btcoexist,
					true, true, 2);
			}
...
static void halbtc8821a2ant_run_coexist_mechanism(struct btc_coexist *btcoexist)
{
	...
	} else {	/* stack doesn't notify profile info.
	...
	)
{
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE,
		...
		((!wifi_busy) ? "idle" :
		((BTC_WIFI_TRAFFIC_TX == wifi_traffic_dir) ? "uplink" : "downlink")));
	CL_PRINTF(cli_buf);
	...
	)
{
	...
	if (dis_ver_info_cnt <= 5) {
		...
		BTC_PRINT(BTC_MSG_INTERFACE, INTF_INIT,
			...
			stack_info->hci_version);
		btcoexist->btc_get(btcoexist,
			BTC_GET_U4_BT_PATCH_VER, &bt_patch_ver);
		btcoexist->btc_get(btcoexist,
			BTC_GET_U4_WIFI_FW_VER, &fw_ver);
		BTC_PRINT(BTC_MSG_INTERFACE, INTF_INIT,
