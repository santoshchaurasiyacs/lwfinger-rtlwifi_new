...
void rtl8723e_dm_bt_set_hw_pta_mode(struct ieee80211_hw *hw, bool b_mode)
{
	...
	if (BT_PTA_MODE_ON == b_mode) {
		...
		rtlpcipriv->btcoexist.hw_coexist_all_off = false;
	} else {
...
void rtl8723e_dm_bt_set_sw_rf_rx_lpf_corner(struct ieee80211_hw *hw, u8 type)
{
	...
	if (BT_RF_RX_LPF_CORNER_SHRINK == type) {
		/* Shrink RF Rx LPF corner,
			0x1e[7:4]=1111 ==> [11:4] by Jenyu */
		RT_TRACE(COMP_BT_COEXIST, DBG_TRACE,
			("Shrink RF Rx LPF corner!!\n"));
		/* PHY_SetRFReg(Adapter,
			(RF_RADIO_PATH_E)PathA, 0x1e, 0xf0, 0xf); */
		rtl8723e_phy_set_rf_reg(hw,
			RF90_PATH_A, 0x1e, 0xfffff, 0xf0ff7);
		rtlpcipriv->btcoexist.sw_coexist_all_off = false;
	} else if (BT_RF_RX_LPF_CORNER_RESUME == type) {
		/*Resume RF Rx LPF corner*/
		RT_TRACE(COMP_BT_COEXIST,
...
bool rtl8723e_dm_bt_is_2_ant_common_action(struct ieee80211_hw *hw)
{
	...
	if (!rtl8723e_dm_bt_is_wifi_busy(hw)
		...
		b_common = true;
	} else if (rtlpcipriv->btcoexist.bt_busy) {
		RT_TRACE(COMP_BT_COEXIST, DBG_DMESG,
			("Bt non-idle!\n"));
		if (mgnt_link_status_query(hw) == RT_MEDIA_CONNECT) {
			RT_TRACE(COMP_BT_COEXIST, DBG_DMESG,
...
void rtl8723e_dm_bt_set_fw_ignore_wlan_act(struct ieee80211_hw *hw,
					bool b_enable)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_pci_priv *rtlpcipriv = rtl_pcipriv(hw);
				...
				struct btdm_8723 *p_btdm)
{
	...
	if (memcmp(p_btdm_8723, p_btdm, sizeof(struct btdm_8723)) == 0) {
		...
		RT_TRACE(COMP_BT_COEXIST, DBG_DMESG,
			...
			p_btdm_8723->b_ignore_wlan_act,
			p_btdm->b_ignore_wlan_act));
	...
	if (rtlpcipriv->btcoexist.hold_for_bt_operation) {
		RT_TRACE(COMP_BT_COEXIST, DBG_TRACE,
	...
	rtl8723e_dm_bt_reject_ap_aggregated_packet(hw,
					p_btdm->b_reject_aggre_pkt);
	...
	rtl8723e_dm_bt_set_fw_wlan_act(hw,
				p_btdm->wlan_act_hi, p_btdm->wlan_act_lo);
	...
	*/
	...
	if (p_btdm->b2_ant_hid_en) {
		...
		rtl8723e_dm_bt_set_fw_tra_tdma_ctrl(hw, p_btdm->b_tra_tdma_on,
							p_btdm->tra_tdma_ant,
							p_btdm->tra_tdma_nav);
		rtl8723e_dm_bt_set_fw_tdma_ctrl(hw, true, p_btdm->tdma_ant,
							p_btdm->tdma_nav,
							p_btdm->tdma_dac_swing);
	...
	if (p_btdm->b_tdma_on) {
		...
		rtl8723e_dm_bt_set_fw_tdma_ctrl(hw, true,
			p_btdm->tdma_ant, p_btdm->tdma_nav,
			p_btdm->tdma_dac_swing);
	} else {
		...
		if (p_btdm->b_ps_tdma_on) {
			rtl8723e_dm_bt_set_fw_3a(hw, p_btdm->ps_tdma_byte[0],
							p_btdm->ps_tdma_byte[1],
				p_btdm->ps_tdma_byte[2], p_btdm->ps_tdma_byte[3],
							p_btdm->ps_tdma_byte[4]);
		} else {
			rtl8723e_dm_bt_set_fw_3a(hw, 0x0, 0x0, 0x0, 0x8, 0x0);
		}
	...
	*/
	...
	rtl8723e_dm_bt_set_sw_full_time_dac_swing(hw,
		p_btdm->sw_dac_swing_on, p_btdm->sw_dac_swing_lvl);
...
void rtl8723e_dm_bt_bt_state_update_2_ant_hid(struct ieee80211_hw *hw)
{
	...
	rtlpcipriv->btcoexist.bt_busy = true;
...
void rtl8723e_dm_bt_2_ant_sco_action(struct ieee80211_hw *hw)
{
	...
	if (rtlphy->current_chan_bw == HT_CHANNEL_WIDTH_20_40) {
		...
		bt_rssi_state
			= rtl8723e_dm_bt_check_coex_rssi_state(hw,
			2, BT_FW_COEX_THRESH_47, 0);
...
void rtl8723e_dm_bt_2_ant_2_dp_action_no_profile(struct ieee80211_hw *hw)
{
	...
	if (rtlphy->current_chan_bw == HT_CHANNEL_WIDTH_20_40) {
		...
		if (rtl8723e_dm_bt_is_wifi_up_link(hw)) {
			...
			btdm8723.b2_ant_hid_en = false;
			/* btSpec = BTHCI_GetBTCoreSpecByProf(Adapter,
				BT_PROFILE_A2DP); */
		...
		if (rtl8723e_dm_bt_is_wifi_up_link(hw)) {
			...
			if ((bt_rssi_state == BT_RSSI_STATE_HIGH) ||
				(bt_rssi_state == BT_RSSI_STATE_STAY_HIGH)) {
				RT_TRACE(COMP_BT_COEXIST,
					DBG_TRACE, ("Wifi rssi high\n"));
				btdm8723.b_agc_table_en = true;
				btdm8723.b_adc_back_off_on = true;
				btdm8723.sw_dac_swing_on = false;
			} else {
				RT_TRACE(COMP_BT_COEXIST,
					DBG_TRACE, ("Wifi rssi low\n"));
				btdm8723.b_agc_table_en = false;
				btdm8723.b_adc_back_off_on = false;
				btdm8723.sw_dac_swing_on = false;
			}
...
void rtl8723e_dm_bt_inq_page_monitor(struct ieee80211_hw *hw)
{
	...
	if (hal_coex_8723.b_c2h_bt_inquiry_page) {
		/* pHalData->btcoexist.halCoex8723.btInquiryPageCnt++; */
	...
	if (rtl8723e_dm_bt_is_2_ant_common_action(hw)) {
		...
		if ((bt_info_original & BTINFO_B_HID) ||
			...
			(bt_info_original & BTINFO_B_SCO_ESCO)) {
				rtlpcipriv->btcoexist.current_state |=
					BT_COEX_STATE_BTINFO_B_HID_SCOESCO;
				rtlpcipriv->btcoexist.bt_profile_case =
...
void rtl8723e_dm_bt_fw_coex_all_off_8723a(struct ieee80211_hw *hw)
{
	...
	rtl8723e_dm_bt_set_fw_3a(hw, 0x0, 0x0, 0x0, 0x8, 0x0);
	rtl8723e_dm_bt_set_fw_2_ant_hid(hw, false, false);
	rtl8723e_dm_bt_set_fw_tra_tdma_ctrl(hw,
		false, TDMA_2ANT, TDMA_NAV_OFF);
...
void rtl8723e_dm_bt_bt_enable_disable_check(struct ieee80211_hw *hw)
{
	...
	if (bt_alife) {
		...
		rtlpcipriv->btcoexist.cur_bt_disabled = false;
		RT_TRACE(COMP_BT_COEXIST,
		...
		if (rtlpcipriv->btcoexist.bt_active_zero_cnt >= 2) {
			rtlpcipriv->btcoexist.cur_bt_disabled = true;
			RT_TRACE(COMP_BT_COEXIST, DBG_TRACE,
				("8723A BT is disabled !!\n"));
		}
	}
	if (rtlpcipriv->btcoexist.pre_bt_disabled !=
		rtlpcipriv->btcoexist.cur_bt_disabled) {
		RT_TRACE(COMP_BT_COEXIST,
			DBG_TRACE, ("8723A BT is from %s to %s!!\n",
			(rtlpcipriv->btcoexist.pre_bt_disabled ?
				"disabled":"enabled"),
			(rtlpcipriv->btcoexist.cur_bt_disabled ?
				"disabled":"enabled")));
		rtlpcipriv->btcoexist.pre_bt_disabled
...
void rtl8723e_dm_bt_parse_bt_info(struct ieee80211_hw *hw,
				u8 *tmp_buf, u8 len)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_pci_priv *rtlpcipriv = rtl_pcipriv(hw);
	...
	if (bt_info & BTINFO_B_CONNECTION) {
		...
		rtlpcipriv->btcoexist.bt_busy = false;
		rtlpcipriv->btcoexist.current_state |= BT_COEX_STATE_BT_IDLE;
	}
}
