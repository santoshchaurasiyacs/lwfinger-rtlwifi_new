		...
		enum radio_path rfpath, u32 offset)
{
	...
	if (rfpath == RF90_PATH_A)
		...
		rfpi_enable =
			(u8)rtl_get_bbreg(hw,
				RFPGA0_XB_HSSIPARAMETER1, BIT(8));
			...
			u32 regaddr, u32 bitmask, u32 data)
{
	...
	if (bitmask != RFREG_OFFSET_MASK) {
		original_value =
			_rtl92s_phy_rf_serial_read(hw, rfpath, regaddr);
		bitshift = _rtl92s_phy_calculate_bit_shift(bitmask);
		...
		enum nl80211_channel_type ch_type)
{
	...
	case HT_CHANNEL_WIDTH_20_40:
		...
		break;
	default:
		RT_TRACE(rtlpriv, COMP_ERR, DBG_EMERG, "unknown bandwidth: %#X\n",
	...
	do {
		...
		case CMDID_WRITEPORT_ULONG:
			rtl_write_dword(rtlpriv,
				currentcmd->para1, currentcmd->para2);
			break;
		case CMDID_WRITEPORT_USHORT:
			rtl_write_word(rtlpriv,
				currentcmd->para1, (u16)currentcmd->para2);
			break;
		case CMDID_WRITEPORT_UCHAR:
			rtl_write_byte(rtlpriv,
				currentcmd->para1, (u8)currentcmd->para2);
			break;
...
static void _rtl92s_phy_init_register_definition(struct ieee80211_hw *hw)
{
	...
	/* RX AFE control 1  */
	rtlphy->phyreg_def[RF90_PATH_A].rfrxiq_imbal =
		ROFDM0_XARXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_B].rfrxiq_imbal =
		ROFDM0_XBRXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_C].rfrxiq_imbal =
		ROFDM0_XCRXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_D].rfrxiq_imbal =
	...
	/* Tx AFE control 1  */
	rtlphy->phyreg_def[RF90_PATH_A].rftxiq_imbal =
		ROFDM0_XATXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_B].rftxiq_imbal =
		ROFDM0_XBTXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_C].rftxiq_imbal =
		ROFDM0_XCTXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_D].rftxiq_imbal =
	...
	rtlphy->phyreg_def[RF90_PATH_C].rf_rb =
		RFPGA0_XC_LSSIREADBACK;
	rtlphy->phyreg_def[RF90_PATH_D].rf_rb =
...
static bool _rtl92s_phy_bb_config_parafile(struct ieee80211_hw *hw)
{
	...
	/* 2. If EEPROM or EFUSE autoload OK,
		We must config by PHY_REG_PG.txt */
	...
	if (rtstatus != true) {
		RT_TRACE(rtlpriv, COMP_INIT, DBG_EMERG,
			"_rtl92s_phy_bb_config_parafile():AGC Table Fail\n");
		goto phy_BB8190_Config_ParaFile_Fail;
	}
	...
	case RF90_PATH_A:
		for (i = 0; i < radio_a_tblen; i = i + 2) {
			if (radio_a_table[i] == 0xfe)
				/* Deay specific ms.
					Only RF configuration require delay. */
				mdelay(50);
	...
	case RF90_PATH_B:
		for (i = 0; i < radio_b_tblen; i = i + 2) {
			if (radio_b_table[i] == 0xfe)
				...
				rtl92s_phy_set_rf_reg(hw,
						rfpath, radio_b_table[i],
						MASK20BITS,
						radio_b_table[i + 1]);
...
static void _rtl92s_phy_get_txpower_index(struct ieee80211_hw *hw,
		u8 channel,
		u8 *cckpowerlevel,
		u8 *ofdmpowerLevel)
{
...
void rtl92s_phy_set_txpower(struct ieee80211_hw *hw, u8	channel)
{
	...
	struct rtl_efuse *rtlefuse = rtl_efuse(rtl_priv(hw));
	/* [0]:RF-A, [1]:RF-B */
	u8 cckpowerlevel[2] = {0}, ofdmpowerLevel[2] = {0};
...
static void _rtl92s_phy_set_fwcmd_io(struct ieee80211_hw *hw)
{
	...
	case FW_CMD_RA_REFRESH_N:
		...
		break;
	case FW_CMD_RA_REFRESH_BG:
		RT_TRACE(rtlpriv, COMP_CMD, DBG_DMESG, "FW_CMD_RA_REFRESH_BG\n");
	...
	case FW_CMD_RA_REFRESH_N_COMB:
		RT_TRACE(rtlpriv, COMP_CMD, DBG_DMESG, "FW_CMD_RA_REFRESH_N_COMB\n");
		input = FW_RA_IOT_N_COMB;
		rtl_write_dword(rtlpriv, WFM5, input);
		rtl92s_phy_chk_fwcmd_iodone(hw);
		break;
	case FW_CMD_RA_REFRESH_BG_COMB:
		RT_TRACE(rtlpriv, COMP_CMD, DBG_DMESG, "FW_CMD_RA_REFRESH_BG_COMB\n");
	...
	case FW_CMD_CTRL_DM_BY_DRIVER:
		RT_TRACE(rtlpriv, COMP_CMD, DBG_LOUD, "FW_CMD_CTRL_DM_BY_DRIVER\n");
...
bool rtl92s_phy_set_fw_cmd(struct ieee80211_hw *hw, enum fwcmd_iotype fw_cmdio)
{
	...
	do {
		...
		case FW_CMD_DIG_DISABLE:
			RT_TRACE(rtlpriv, COMP_CMD, DBG_LOUD, "Set DIG disable!!\n");
		...
		case FW_CMD_DIG_HALT:
			RT_TRACE(rtlpriv, COMP_CMD, DBG_LOUD, "Set DIG halt!!\n");
			fw_cmdmap &= ~(FW_DIG_ENABLE_CTL | FW_SS_CTL);
			FW_CMD_IO_SET(rtlpriv, fw_cmdmap);
			break;
