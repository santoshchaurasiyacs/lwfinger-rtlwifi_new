		...
		u8 chnl, u32 *ofdmbase,	u32 *mcsbase, u8 *p_final_pwridx)
{
	...
	if (rtlefuse->eeprom_version < 2) {
		...
		legacy_pwrdiff =
			rtlefuse->txpwr_legacyhtdiff[RF90_PATH_A][chnl - 1];
	...
	if (rtlefuse->eeprom_version >= 2) {
		...
		if (rtlphy->current_chan_bw == HT_CHANNEL_WIDTH_20) {
			for (i = 0; i < 2; i++) {
				...
				/* HT 20<->40 pwr diff */
				ht20_pwrdiff =
					rtlefuse->txpwr_ht20diff[i][chnl - 1];
	...
	*mcsbase = pwrbase1;
	...
	case 3:
		...
		if (rtlphy->current_chan_bw == HT_CHANNEL_WIDTH_20_40) {
			p_final_pwridx[0] +=
				rtlefuse->pwrgroup_ht40[RF90_PATH_A][
						chnl - 1];
			p_final_pwridx[1] +=
				rtlefuse->pwrgroup_ht40[RF90_PATH_B][
						chnl - 1];
		} else {
			p_final_pwridx[0] +=
				rtlefuse->pwrgroup_ht20[RF90_PATH_A][
						chnl - 1];
			p_final_pwridx[1] +=
				rtlefuse->pwrgroup_ht20[RF90_PATH_B][
						chnl - 1];
		}
		break;
	default:
		...
		u8 *p_final_pwridx)
{
	...
	if (rtlphy->rf_type == RF_2T2R) {
		...
		RT_TRACE(rtlpriv, COMP_POWER, DBG_LOUD, ("Antenna Diff from RF-B "
			"to RF-A = %d (0x%x)\n",
...
static void _rtl92s_get_txpower_writeval_byregulatory(struct ieee80211_hw *hw,
		u8 chnl, u8 index, u32 pwrbase0,
		u32 pwrbase1, u32 *p_outwrite_val)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &(rtlpriv->phy);
	struct rtl_efuse *rtlefuse = rtl_efuse(rtl_priv(hw));
	u8 i, chnlgroup = 0, pwrdiff_limit[4];
	...
	case 0:
		...
		RT_TRACE(rtlpriv, COMP_POWER, DBG_LOUD, ("RTK better performance, "
				"writeval = 0x%x\n", writeval));
		break;
	case 1:
		...
		if (rtlphy->current_chan_bw == HT_CHANNEL_WIDTH_20_40) {
			...
			RT_TRACE(rtlpriv, COMP_POWER, DBG_LOUD, ("Realtek regulatory, "
					"40MHz, writeval = 0x%x\n", writeval));
		} else {
	...
	case 2:
		...
		writeval = ((index < 2) ? pwrbase0 : pwrbase1);
		RT_TRACE(rtlpriv, COMP_POWER, DBG_LOUD, ("Better regulatory, "
				"writeval = 0x%x\n", writeval));
		break;
	case 3:
		...
		customer_limit = (pwrdiff_limit[3] << 24) |
				...
				(pwrdiff_limit[1] << 8) |
				(pwrdiff_limit[0]);
		RT_TRACE(rtlpriv, COMP_POWER, DBG_LOUD, ("Customer's limit = "
				"0x%x\n", customer_limit));

		writeval = customer_limit +
				((index < 2) ? pwrbase0 : pwrbase1);
		RT_TRACE(rtlpriv, COMP_POWER, DBG_LOUD, ("Customer, writeval = "
				"0x%x\n", writeval));
		break;
	default:
		chnlgroup = 0;
...
bool rtl92s_phy_rf6052_config(struct ieee80211_hw *hw)
{
	...
	for (rfpath = 0; rfpath < rtlphy->num_total_rfpath; rfpath++) {
		...
		rtl92s_phy_set_bb_reg(hw,
			pphyreg->rfintfe, BRFSI_RFENV << 16, 0x1);
...
void rtl92s_phy_rf6052_set_bandwidth(struct ieee80211_hw *hw, u8 bandwidth)
{
	...
	case HT_CHANNEL_WIDTH_20:
		rtlphy->rfreg_chnlval[0] =
			((rtlphy->rfreg_chnlval[0] & 0xfffff3ff) |
				0x0400);
		rtl_set_rfreg(hw,
			RF90_PATH_A, RF_CHNLBW, RFREG_OFFSET_MASK,
				rtlphy->rfreg_chnlval[0]);
		break;
