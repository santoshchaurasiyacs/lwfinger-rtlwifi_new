...
void rtl88ee_set_hw_reg(struct ieee80211_hw *hw, u8 variable, u8 *val)
{
	...
	case HW_VAR_KEEP_ALIVE: {
			...
			array[0] = 0xff;
			array[1] = *((u8 *)val);
			rtl88e_fill_h2c_cmd(hw, H2C_88E_KEEP_ALIVE_CTRL, 2, array);
			break;
		}
	default:
...
static bool _rtl88ee_llt_table_init(struct ieee80211_hw *hw)
{
	...
	/*0x2600   MaxRxBuff=10k-max(TxReportSize(64*8),
	WOLPattern(16*24)). YJ,111031*/
	...
	u8 *hwinfo)
{
	...
	RT_TRACE(COMP_INIT, DBG_LOUD, ("hal_ReadPowerValueFromPROM88E():"
						 ...
						 u8 *hwinfo)
{
	...
	_rtl8188e_read_power_value_fromprom(hw, &pwrinfo24g,
...
static void _rtl88ee_read_adapter_info(struct ieee80211_hw *hw)
{
	...
	/*board type*/
	rtlefuse->board_type =
		(((*(u8 *) &hwinfo[EEPROM_RF_BOARD_OPTION_88E]) & 0xE0) >> 5);
