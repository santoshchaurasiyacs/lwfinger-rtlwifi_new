...
#define		CALCULATE_SWINGTALBE_OFFSET(_offset, \
					_direction, _size, _deltaThermal) \
					do {\
	...
	u8 type, u8 *pdirection, u32 *poutwrite_val)
{
	...
	if (type == 0) {
		...
		if (rtldm->swing_idx_cck <= rtldm->swing_idx_cck_base) {
			*pdirection = 1;
			pwr_val = rtldm->swing_idx_cck_base - rtldm->swing_idx_cck;
		} else {
			*pdirection = 2;
			pwr_val = rtldm->swing_idx_cck - rtldm->swing_idx_cck_base;
		}
	}
	...
	if (method == TXAGC) {
		...
		if (!rtldm->cck_inch14) {
			rtl_write_byte(rtlpriv,
				0xa22, cckswing_table_ch1ch13[rtldm->swing_idx_cck][0]);
			rtl_write_byte(rtlpriv,
				0xa23, cckswing_table_ch1ch13[rtldm->swing_idx_cck][1]);
			rtl_write_byte(rtlpriv,
				0xa24, cckswing_table_ch1ch13[rtldm->swing_idx_cck][2]);
			rtl_write_byte(rtlpriv,
				0xa25, cckswing_table_ch1ch13[rtldm->swing_idx_cck][3]);
			rtl_write_byte(rtlpriv,
				0xa26, cckswing_table_ch1ch13[rtldm->swing_idx_cck][4]);
			rtl_write_byte(rtlpriv,
				0xa27, cckswing_table_ch1ch13[rtldm->swing_idx_cck][5]);
			rtl_write_byte(rtlpriv,
				0xa28, cckswing_table_ch1ch13[rtldm->swing_idx_cck][6]);
			rtl_write_byte(rtlpriv,
				0xa29, cckswing_table_ch1ch13[rtldm->swing_idx_cck][7]);
		} else{
			rtl_write_byte(rtlpriv,
				0xa22, cckswing_table_ch14[rtldm->swing_idx_cck][0]);
			rtl_write_byte(rtlpriv,
				0xa23, cckswing_table_ch14[rtldm->swing_idx_cck][1]);
			rtl_write_byte(rtlpriv,
				0xa24, cckswing_table_ch14[rtldm->swing_idx_cck][2]);
			rtl_write_byte(rtlpriv,
				0xa25, cckswing_table_ch14[rtldm->swing_idx_cck][3]);
			rtl_write_byte(rtlpriv,
				0xa26, cckswing_table_ch14[rtldm->swing_idx_cck][4]);
			rtl_write_byte(rtlpriv,
				0xa27, cckswing_table_ch14[rtldm->swing_idx_cck][5]);
			rtl_write_byte(rtlpriv,
				0xa28, cckswing_table_ch14[rtldm->swing_idx_cck][6]);
			rtl_write_byte(rtlpriv,
				0xa29, cckswing_table_ch14[rtldm->swing_idx_cck][7]);
		}
	...
	if (rtlpriv->falsealm_cnt.cnt_all > 10000) {
		...
		if (dm_dig->recover_cnt != 0)
			...
			if (dm_dig->large_fa_hit == 0) {
				if ((dm_dig->forbidden_igi - 1) < dig_dynamic_min) {
...
static void rtl88e_dm_txpower_tracking_callback_thermalmeter(
	struct ieee80211_hw *hw)
{
	...
	if (delta > 0 && rtlpriv->dm.txpower_track_control) {
		...
		if (thermalvalue > rtlefuse->eeprom_thermalmeter) {
			CALCULATE_SWINGTALBE_OFFSET(offset,
				power_inc, index_mapping_NUM, delta);
			for (i = 0; i < rf; i++)
			...
			CALCULATE_SWINGTALBE_OFFSET(offset,
				power_dec, index_mapping_NUM, delta);
			for (i = 0; i < rf; i++)
				ofdm_index[i] = rtldm->ofdm_index[i] +
		...
		if (rtlpriv->dm.txpower_track_control) {
			...
			if (rtldm->swing_idx_cck_current != rtldm->swing_idx_cck) {
				rtldm->swing_idx_cck_current = rtldm->swing_idx_cck;
				rtldm->swing_flag_cck = true;
			}
	...
	u8 ant)
{
	...
	if (pfat_table->rx_idle_ant != ant) {
		RT_TRACE(rtlpriv, COMP_INIT, DBG_LOUD, "need to update rx idle ant\n");
		if (ant == MAIN_ANT) {
			default_ant = (pfat_table->rx_idle_ant == CG_TRX_HW_ANTDIV) ?
				MAIN_ANT_CG_TRX : MAIN_ANT_CGCS_RX;
			optional_ant = (pfat_table->rx_idle_ant == CG_TRX_HW_ANTDIV) ?
...
static void rtl88e_dm_hw_ant_div(struct ieee80211_hw *hw)
{
	...
	RT_TRACE(rtlpriv, COMP_INIT, DBG_LOUD, "aux_ant_sum %d aux_ant_cnt %d\n",
...
static void rtl88e_dm_fast_ant_training(struct ieee80211_hw *hw)
{
	...
	if (pfat_table->fat_state == FAT_TRAINING_STATE) {
		...
		if (bpkt_filter_match == false) {
			rtl_set_bbreg(hw,
				DM_REG_TXAGC_A_1_MCS32_11N, BIT(16), 0);
			rtl_set_bbreg(hw,
				DM_REG_IGI_A_11N, BIT(7), 0);
		} else {
			rtl_set_bbreg(hw,
				DM_REG_TXAGC_A_1_MCS32_11N, BIT(16), 0);
			...
			rtl_set_bbreg(hw,
				DM_REG_TX_ANT_CTRL_11N, BIT(21), 1);
...
static void rtl88e_dm_antenna_diversity(struct ieee80211_hw *hw)
{
	...
	if (mac->link_state < MAC80211_LINKED) {
		RT_TRACE(rtlpriv, COMP_DIG, DBG_LOUD, "No Link\n");
		if (pfat_table->becomelinked == true) {
			RT_TRACE(rtlpriv, COMP_DIG, DBG_LOUD,
		...
		if (pfat_table->becomelinked == false) {
			RT_TRACE(rtlpriv, COMP_DIG, DBG_LOUD,
