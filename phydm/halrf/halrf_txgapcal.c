...
)
{
	...
	u4Byte psd_report_addr[2] = {BBREG_0xea0, BBREG_0xec0};
	
	odm_set_rf_reg(p_dm_odm, rf_path, RFREG_0xdf, bRFRegOffsetMask, 0x00e02);

	ODM_delay_us(100);

	odm_set_bb_reg(p_dm_odm, BBREG_0xe28, 0xffffffff, 0x0);
	
	odm_set_rf_reg(p_dm_odm, rf_path, RFREG_0x56, 0xfff, rf56);
	while(rf56 != (odm_get_rf_reg(p_dm_odm, rf_path, RFREG_0x56, 0xfff)))
		odm_set_rf_reg(p_dm_odm, rf_path, RFREG_0x56, 0xfff, rf56);

	odm_set_bb_reg(p_dm_odm, BBREG_0xd94, 0xffffffff, 0x44FFBB44);
	odm_set_bb_reg(p_dm_odm, BBREG_0xe70, 0xffffffff, 0x00400040);
	odm_set_bb_reg(p_dm_odm, BBREG_0xc04, 0xffffffff, 0x6f005403);
	odm_set_bb_reg(p_dm_odm, BBREG_0xc08, 0xffffffff, 0x000804e4);
	odm_set_bb_reg(p_dm_odm, BBREG_0x874, 0xffffffff, 0x04203400);
	odm_set_bb_reg(p_dm_odm, BBREG_0xe28, 0xffffffff, 0x80800000);
	...
	odm_set_bb_reg(p_dm_odm, BBREG_0xe40, 0xffffffff, 0x81007C00);
	odm_set_bb_reg(p_dm_odm, BBREG_0xe44, 0xffffffff, 0x81004800);
	odm_set_bb_reg(p_dm_odm, BBREG_0xe4c, 0xffffffff, 0x0046a8d0);
	

	for (i = 0; i < psd_avg_time; i++) {
			
		for(j = 0; j < 1000 ; j++) {
			odm_set_bb_reg(p_dm_odm, BBREG_0xe48, 0xffffffff, 0xfa005800);
			odm_set_bb_reg(p_dm_odm, BBREG_0xe48, 0xffffffff, 0xf8005800);

			while(!odm_get_bb_reg(p_dm_odm, BBREG_0xeac, psd_finish_bit[rf_path]));	/*wait finish bit*/

			if (!odm_get_bb_reg(p_dm_odm, BBREG_0xeac, psd_fail_bit[rf_path])) {	/*check fail bit*/
		...
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, 
			...
			rf56, odm_get_rf_reg(p_dm_odm, rf_path, RFREG_0x56, 0xfff), j, psd_vaule[i]));
	}
	...
	odm_set_bb_reg(p_dm_odm, BBREG_0xd94, 0xffffffff, 0x44BBBB44);
	odm_set_bb_reg(p_dm_odm, BBREG_0xe70, 0xffffffff, 0x80408040);
	odm_set_bb_reg(p_dm_odm, BBREG_0xc04, 0xffffffff, 0x6f005433);
	odm_set_bb_reg(p_dm_odm, BBREG_0xc08, 0xffffffff, 0x000004e4);
	odm_set_bb_reg(p_dm_odm, BBREG_0x874, 0xffffffff, 0x04003400);
	odm_set_bb_reg(p_dm_odm, BBREG_0xe28, 0xffffffff, 0x00000000);

	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, 
		("[TGGC] rf0=0x%x rf56=0x%x rf56_reg=0x%x psd_vaule_temp=0x%x\n",
		odm_get_rf_reg(p_dm_odm, rf_path, 0x0, 0xff),
		rf56, odm_get_rf_reg(p_dm_odm, rf_path, RFREG_0x56, 0xfff), psd_vaule_temp));
	
	odm_set_rf_reg(p_dm_odm, rf_path, RFREG_0xdf, bRFRegOffsetMask, 0x00602);
...
)
{
	...
	for (rf_path = RF_PATH_A; rf_path <= RF_PATH_B; rf_path++) {
		...
		if (rf_path == RF_PATH_A)
			odm_set_bb_reg(p_dm_odm, BBREG_0x88c, (BIT(21) | BIT(20)), 0x3);	/*disable 3-wire*/
		else if (rf_path == RF_PATH_B)
			odm_set_bb_reg(p_dm_odm, BBREG_0x88c, (BIT(23) | BIT(22)), 0x3);	/*disable 3-wire*/
		...
		for (rf0_idx = 1; rf0_idx <= 10; rf0_idx++) {
			...
			ODM_delay_us(100);
			rf56_current_temp[rf_path][rf0_idx] = odm_get_rf_reg(p_dm_odm, rf_path, RFREG_0x56, 0xfff);
			...
			ODM_delay_us(100);
			rf56_next= odm_get_rf_reg(p_dm_odm, rf_path, RFREG_0x56, 0xfff);
		...
		if (rf_path == RF_PATH_A)
			odm_set_bb_reg(p_dm_odm, BBREG_0x88c, (BIT(21) | BIT(20)), 0x0);	/*enable 3-wire*/
		else if (rf_path == RF_PATH_B)
			odm_set_bb_reg(p_dm_odm, BBREG_0x88c, (BIT(23) | BIT(22)), 0x0);	/*enable 3-wire*/
	...
	for (rf_path = RF_PATH_A; rf_path <= RF_PATH_B; rf_path++) {
		...
		odm_set_rf_reg(p_dm_odm, rf_path, RFREG_0xef, bRFRegOffsetMask, 0x00100);
		...
		for (rf0_idx = 1; rf0_idx <= 10; rf0_idx++) {
			...
			odm_set_rf_reg(p_dm_odm, rf_path, RFREG_0x33, bRFRegOffsetMask, rf33[rf_path][rf0_idx]);
		}
		
		odm_set_rf_reg(p_dm_odm, rf_path, RFREG_0xef, bRFRegOffsetMask, 0x00000);
	}

}
#endif

void odm_tx_gain_gap_calibration(void *p_dm_void)
{
	PDM_ODM_T p_dm_odm = (PDM_ODM_T)p_dm_void;
