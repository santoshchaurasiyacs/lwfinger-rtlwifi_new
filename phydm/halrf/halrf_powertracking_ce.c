...
u8 get_swing_index(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type == ODM_RTL8188E ||
		...
		bb_swing = odm_get_bb_reg(p_dm, REG_OFDM_0_XA_TX_IQ_IMBALANCE,
					  0xFFC00000);
...
u8 get_cck_swing_index(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type == ODM_RTL8188E ||
	    p_dm->support_ic_type == ODM_RTL8723B ||
	    p_dm->support_ic_type == ODM_RTL8192E) {
		bb_cck_swing = odm_read_1byte(p_dm, BBREG_0xa22);
	...
	if (p_rf_calibrate_info->default_bb_swing_index_flag != true) {
		...
		if (p_dm->support_ic_type == ODM_RTL8188E ||
			...
			p_rf_calibrate_info->default_cck_index =
				(default_cck_swing_index >= CCK_TABLE_SIZE) ?
					20 :
					default_cck_swing_index;
	...
	if (!p_dm->rf_calibrate_info.tm_trigger) {
		if (IS_HARDWARE_TYPE_8188E(adapter) ||
		    ...
		    (p_dm->support_ic_type ==
		     ODM_RTL8710B)) /* JJ ADD 20161014 */
			odm_set_rf_reg(p_dm, RF_PATH_A, RF_T_METER_NEW,
				       (BIT(17) | BIT(16)), 0x03);
		else
			odm_set_rf_reg(p_dm, RF_PATH_A, RF_T_METER_OLD,
