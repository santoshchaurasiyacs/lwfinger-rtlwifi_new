					   ...
					   u8 rate_section, u8 txnum, u8 value)
{
	...
	if (path > RF90_PATH_D) {
		RT_TRACE(rtlpriv, COMP_INIT, DBG_LOUD, "Invalid Rf Path %d\n", path);
		return;
	}
	...
	if (band == BAND_ON_2_4G) {
		...
		RT_TRACE(rtlpriv, COMP_INIT, DBG_LOUD, "Invalid Band %d\n", band);
	}
}
				...
				enum radio_path rf, u8 rate)
{
	...
	if (tx_num == RF_TX_NUM_NONIMPLEMENT) {
		if ((rate >= DESC92_RATEMCS8 && rate <= DESC92_RATEMCS15))
			tx_num = RF_2TX;
		else
			tx_num = RF_1TX;
	}
				     ...
				     u8 bw, u8 channel)
{
	...
	else if (DESC92_RATE6M <= rate)
		tx_power = rtlefuse->txpwrlevel_ht40_1s[rfpath][index];

	/* OFDM-1T*/
	if (DESC92_RATE6M <= rate && rate <= DESC92_RATE54M &&
	    !IS_CCK_RATE(rate))
		tx_power += rtlefuse->txpwr_legacyhtdiff[rfpath][TX_1S];

	/* BW20-1S, BW20-2S */
	if (bw == HT_CHANNEL_WIDTH_20) {
		if (DESC92_RATEMCS0 <= rate && rate <= DESC92_RATEMCS15)
			tx_power += rtlefuse->txpwr_ht20diff[rfpath][TX_1S];
		if (DESC92_RATEMCS8 <= rate && rate <= DESC92_RATEMCS15)
				...
				enum radio_path rfpath, u8 rate)
{
	...
	if (rfpath == RF90_PATH_A) {
		...
		case DESC92_RATE1M:
			rtl_set_bbreg(hw,
				 RTXAGC_A_CCK1_MCS32, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATE2M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATE5_5M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATE11M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATE6M:
			rtl_set_bbreg(hw,
				 RTXAGC_A_RATE18_06, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATE9M:
			rtl_set_bbreg(hw,
				 RTXAGC_A_RATE18_06, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATE12M:
			rtl_set_bbreg(hw,
				 RTXAGC_A_RATE18_06, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATE18M:
			rtl_set_bbreg(hw,
				 RTXAGC_A_RATE18_06, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATE24M:
			rtl_set_bbreg(hw,
				 RTXAGC_A_RATE54_24, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATE36M:
			rtl_set_bbreg(hw,
				 RTXAGC_A_RATE54_24, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATE48M:
			rtl_set_bbreg(hw,
				 RTXAGC_A_RATE54_24, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATE54M:
			rtl_set_bbreg(hw,
				 RTXAGC_A_RATE54_24, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATEMCS0:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS03_MCS00, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATEMCS1:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS03_MCS00, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATEMCS2:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS03_MCS00, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATEMCS3:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS03_MCS00, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATEMCS4:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS07_MCS04, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATEMCS5:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS07_MCS04, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATEMCS6:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS07_MCS04, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATEMCS7:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS07_MCS04, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATEMCS8:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS11_MCS08, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATEMCS9:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS11_MCS08, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATEMCS10:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS11_MCS08, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATEMCS11:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS11_MCS08, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATEMCS12:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS15_MCS12, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATEMCS13:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS15_MCS12, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATEMCS14:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS15_MCS12, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATEMCS15:
			rtl_set_bbreg(hw,
				 RTXAGC_A_MCS15_MCS12, MASKBYTE3, pwr_idx);
			break;
		...
		case DESC92_RATE1M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_CCK1_55_MCS32, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATE2M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_CCK1_55_MCS32, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATE5_5M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_CCK1_55_MCS32, MASKBYTE3, pwr_idx);
			break;
		case DESC92_RATE11M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE0, pwr_idx);
			break;

		case DESC92_RATE6M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_RATE18_06, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATE9M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_RATE18_06, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATE12M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_RATE18_06, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATE18M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_RATE18_06, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATE24M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_RATE54_24, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATE36M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_RATE54_24, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATE48M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_RATE54_24, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATE54M:
			rtl_set_bbreg(hw,
				 RTXAGC_B_RATE54_24, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATEMCS0:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS03_MCS00, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATEMCS1:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS03_MCS00, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATEMCS2:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS03_MCS00, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATEMCS3:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS03_MCS00, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATEMCS4:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS07_MCS04, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATEMCS5:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS07_MCS04, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATEMCS6:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS07_MCS04, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATEMCS7:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS07_MCS04, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATEMCS8:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS11_MCS08, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATEMCS9:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS11_MCS08, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATEMCS10:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS11_MCS08, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATEMCS11:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS11_MCS08, MASKBYTE3, pwr_idx);
			break;

		case DESC92_RATEMCS12:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS15_MCS12, MASKBYTE0, pwr_idx);
			break;
		case DESC92_RATEMCS13:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS15_MCS12, MASKBYTE1, pwr_idx);
			break;
		case DESC92_RATEMCS14:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS15_MCS12, MASKBYTE2, pwr_idx);
			break;
		case DESC92_RATEMCS15:
			rtl_set_bbreg(hw,
				 RTXAGC_B_MCS15_MCS12, MASKBYTE3, pwr_idx);
			break;
						   ...
						   enum rate_section section)
{
	...
	if (section == CCK) {
		u8 cck_rates[] = {DESC92_RATE1M, DESC92_RATE2M,
				  DESC92_RATE5_5M, DESC92_RATE11M};
		if (rtlhal->current_bandtype == BAND_ON_2_4G)
			rtl92ee_phy_set_txpower_index_by_rate_array(hw, rfpath,
							rtlphy->current_chan_bw,
							channel, cck_rates, 4);
	} else if (section == OFDM) {
		u8 ofdm_rates[] = {DESC92_RATE6M, DESC92_RATE9M,
				   DESC92_RATE12M, DESC92_RATE18M,
				   DESC92_RATE24M, DESC92_RATE36M,
				   DESC92_RATE48M, DESC92_RATE54M};
		rtl92ee_phy_set_txpower_index_by_rate_array(hw, rfpath,
							rtlphy->current_chan_bw,
							channel, ofdm_rates, 8);
	} else if (section == HT_MCS0_MCS7) {
		u8 ht_rates1t[]  = {DESC92_RATEMCS0, DESC92_RATEMCS1,
				    DESC92_RATEMCS2, DESC92_RATEMCS3,
				    DESC92_RATEMCS4, DESC92_RATEMCS5,
				    DESC92_RATEMCS6, DESC92_RATEMCS7};
		rtl92ee_phy_set_txpower_index_by_rate_array(hw, rfpath,
							rtlphy->current_chan_bw,
							channel, ht_rates1t, 8);
	} else if (section == HT_MCS8_MCS15) {
		u8 ht_rates2t[]  = {DESC92_RATEMCS8, DESC92_RATEMCS9,
				    DESC92_RATEMCS10, DESC92_RATEMCS11,
				    DESC92_RATEMCS12, DESC92_RATEMCS13,
				    DESC92_RATEMCS14, DESC92_RATEMCS15};
		rtl92ee_phy_set_txpower_index_by_rate_array(hw, rfpath,
							rtlphy->current_chan_bw,
				      ...
				      long result[][8], u8 t, bool is2t)
{
	...
	if (0x00 == patha_ok)
		RT_TRACE(rtlpriv, COMP_RF, DBG_LOUD, "Path A IQK failed!!, ret = 0\n");
	...
	RT_TRACE(rtlpriv, COMP_RF, DBG_LOUD, "_rtl92ee_phy_iq_calibrate() <==\n");
}
