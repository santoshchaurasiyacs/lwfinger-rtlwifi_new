...
void rtl8723be_set_hw_reg(struct ieee80211_hw *hw, u8 variable, u8 *val)
{
	...
	case HW_VAR_KEEP_ALIVE:{
		...
		break;
	default:
		RT_TRACE(rtlpriv, COMP_ERR, DBG_LOUD, "switch case not process %x\n",
			 variable);
		break;
	}
}
					 ...
					 bool mac_power_on)
{
	...
	RT_TRACE(rtlpriv, COMP_INIT, DBG_LOUD, "ResetPcieInterfaceDMA8723BE()\n");
...
int rtl8723be_hw_init(struct ieee80211_hw *hw)
{
	...
	if (ppsc->rfpwr_state == ERFON) {
		...
		if (rtlpriv->btcoexist.btc_info.ant_num == ANT_X2 ||
			...
			rtl8723be_phy_iq_calibrate(hw, (rtlphy->iqk_initialized ? true : false));
...
static enum version_8723be _rtl8723be_read_chip_version(struct ieee80211_hw *hw)
{
	...
	if ((value32 & (CHIP_8723B)) != CHIP_8723B)
		...
		version = (enum version_8723be) CHIP_8723B;

	rtlphy->rf_type = RF_1T1R;

	/* treat rtl8723be chip as  MP version in default */
	version = (enum version_8723be) (version | NORMAL_CHIP);

	value32 = rtl_read_dword(rtlpriv, REG_SYS_CFG);
	/* cut version */
	version |= (enum version_8723be) (value32 & CHIP_VER_RTL_MASK);
	/* Manufacture */
	if (((value32 & EXT_VENDOR_ID) >> 18) == 0x01)
		version = (enum version_8723be) (version | CHIP_VENDOR_SMIC);
