...
static bool _rtl8723de_check_fw_read_last_h2c(struct ieee80211_hw *hw,
					      u8 boxnum)
{
...
static void _rtl8723de_fill_h2c_command(struct ieee80211_hw *hw, u8 element_id,
					u32 cmd_len, u8 *p_cmdbuffer)
{
	...
	RT_TRACE(rtlpriv, COMP_CMD, DBG_DMESG,
		 "H2C: %02X %02X %02X %02X %02X %02X %02X %02X (len=%d)\n",
		 element_id, p_cmdbuffer[0], p_cmdbuffer[1], p_cmdbuffer[2],
		 p_cmdbuffer[3], p_cmdbuffer[4], p_cmdbuffer[5], p_cmdbuffer[6],
		 cmd_len);
	...
	while (!bwrite_sucess) {
		...
		isfw_read = _rtl8723de_check_fw_read_last_h2c(hw, boxnum);
		while (!isfw_read) {
			wait_h2c_limmit--;
			...
			isfw_read = _rtl8723de_check_fw_read_last_h2c(hw,
								boxnum);
...
void rtl8723de_fill_h2c_cmd(struct ieee80211_hw *hw, u8 element_id,
			    u32 cmd_len, u8 *p_cmdbuffer)
{
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
	u32 tmp_cmdbuf[2];
	...
	if (!rtlhal->fw_ready) {
		WARN_ONCE(true,
			  "rtl8723de: error H2C cmd because of Fw download fail!!!\n");
		return;
	}

	memset(tmp_cmdbuf, 0, 8);
	memcpy(tmp_cmdbuf, p_cmdbuffer, cmd_len);
	_rtl8723de_fill_h2c_command(hw, element_id, cmd_len,
				    (u8 *)&tmp_cmdbuf);
	return;
}

void rtl8723de_set_fw_pwrmode_cmd(struct ieee80211_hw *hw, u8 mode)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	...
	if (rtlpriv->cfg->ops->get_btc_status())
		btc_ops->btc_record_pwr_mode(rtlpriv, u1_h2c_set_pwrmode,
					     H2C_PWEMODE_LENGTH);
	rtl8723de_fill_h2c_cmd(hw, H2C_8723D_SETPWRMODE, H2C_PWEMODE_LENGTH,
			       u1_h2c_set_pwrmode);
}

void rtl8723de_set_fw_media_status_rpt_cmd(struct ieee80211_hw *hw, u8 mstatus)
{
	u8 parm[3] = { 0, 0, 0 };
	...
	*/
	...
	rtl8723de_fill_h2c_cmd(hw, H2C_8723D_MSRRPT, 3, parm);
}

#if 0
void rtl8723de_inform_fw_iqk(struct ieee80211_hw *hw, bool start)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	int retry = 10;
	u8 cmd_data = start ? 1 : 0;

	rtl8723de_fill_h2c_cmd(hw, H2C_8723D_BT_WLAN_CALIBRATION, 1, &cmd_data);
	...
	/* wait for ack */
	do {
		if (rtl_read_byte(rtlpriv, REG_GENTST_3) & BIT_GENTST_BT_ACK)
			break;
...
static bool _rtl8723de_send_bcn_packet(struct ieee80211_hw *hw,
				       struct sk_buff *skb)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	...
	u8 *pdesc_or_bddesc;
	dma_addr_t dma_addr;
	int hw_queue = BEACON_QUEUE;

	ring = &rtlpci->tx_ring[hw_queue];

	spin_lock_irqsave(&rtlpriv->locks.irq_th_lock, flags);

	/* get beacon desc and bd */
	pdesc = &ring->desc[0];
...
void rtl8723de_set_fw_rsvdpagepkt(struct ieee80211_hw *hw,
				  bool b_dl_finished)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	...
	RT_PRINT_DATA(rtlpriv, COMP_CMD, DBG_LOUD,
		      "rtl8723de_set_fw_rsvdpagepkt(): HW_VAR_SET_TX_CMD: ALL\n",
	...
	rtstatus = _rtl8723de_send_bcn_packet(hw, skb);
	...
	if (b_dlok) {
		...
		RT_PRINT_DATA(rtlpriv, COMP_CMD, DBG_DMESG, "H2C_RSVDPAGE:\n",
			      u1rsvdpageloc, sizeof(u1rsvdpageloc));
		rtl8723de_fill_h2c_cmd(hw, H2C_8723D_RSVDPAGE,
...
void rtl8723de_set_p2p_ps_offload_cmd(struct ieee80211_hw *hw,
				      u8 p2p_ps_state)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	...
	case P2P_PS_ENABLE:
		...
		if (p2pinfo->ctwindow > 0) {
			...
			rtl_write_byte(rtlpriv, REG_P2P_CTWIN, ctwindow);
		}
	...
	rtl8723de_fill_h2c_cmd(hw, H2C_8723D_P2P_PS_OFFLOAD, 1,
			       (u8 *)p2p_ps_offload);
}

void rtl8723de_c2h_content_parsing(struct ieee80211_hw *hw,
				   u8 c2h_cmd_id,
				   u8 c2h_cmd_len, u8 *tmp_buf)
{
	...
	case C2H_8723D_DBG:
		RT_TRACE(rtlpriv, COMP_FW, DBG_TRACE,
			 "[C2H], C2H_8723DE_DBG!!\n");
		break;
	case C2H_8723D_TX_REPORT:
		RT_TRACE(rtlpriv, COMP_FW, DBG_TRACE,
			 "[C2H], C2H_8723DE_TX_REPORT!\n");
		rtl_tx_report_handler(hw, tmp_buf, c2h_cmd_len);
		break;
	case C2H_8723D_BT_INFO:
		RT_TRACE(rtlpriv, COMP_FW, DBG_TRACE,
			 "[C2H], C2H_8723DE_BT_INFO!!\n");
		if (rtlpriv->cfg->ops->get_btc_status())
			btc_ops->btc_btinfo_notify(rtlpriv, tmp_buf,
						   c2h_cmd_len);
		break;
	case C2H_8723D_BT_MP:
		RT_TRACE(rtlpriv, COMP_FW, DBG_TRACE,
			 "[C2H], C2H_8723DE_BT_MP!!\n");
		if (rtlpriv->cfg->ops->get_btc_status())
...
void rtl8723de_c2h_packet_handler(struct ieee80211_hw *hw, u8 *buffer, u8 len)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	...
	default:
		rtl8723de_c2h_content_parsing(hw, c2h_cmd_id, c2h_cmd_len,
					      tmp_buf);
		break;
	}
}
